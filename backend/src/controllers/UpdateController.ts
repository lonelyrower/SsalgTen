  ...
  async triggerUpdate(req: Request, res: Response) {
    ...
    try {
      ...
      return res.status(501).json({
        success: false,
        error:
          "Updater service not reachable. Please deploy ssalgten-updater on host.",
        data: {
          hint: "Build and run updater: Dockerfile.updater -> container on host port 8765",
          env: {
            UPDATER_URL:
              process.env.UPDATER_URL ||
              "http://host.docker.internal:8765/update",
          },
        },
      });
    }
  ...
  async updaterHealth(req: Request, res: Response) {
    ...
    try {
      ...
      if (!resp.ok) {
        return res
          .status(resp.status)
          .json({ success: false, error: `updater health ${resp.status}` });
      }
