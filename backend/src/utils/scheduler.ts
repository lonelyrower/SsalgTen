...
const OFFLINE_CHECK_INTERVAL_MINUTES = parseInt(process.env.OFFLINE_CHECK_INTERVAL_MINUTES || '5');
...
export const startSchedulers = () => {
  ...
  logger.info(`  - 每 ${CLEANUP_INTERVAL_HOURS}h 清理超过 ${HEARTBEAT_RETENTION_DAYS} 天的旧记录`);
  logger.info(`  - 每 ${OFFLINE_CHECK_INTERVAL_MINUTES} 分钟检查离线节点 (超过 ${OFFLINE_THRESHOLD_MINUTES} 分钟无心跳)`);
