...
export const UnifiedDashboardPage: React.FC = () => {
  ...
  if (!connected && nodes.length === 0) {
    return (
      <div className="fixed inset-0 flex flex-col bg-[hsl(var(--background))] transition-colors duration-[var(--duration-normal)]">
        <Header />
        <LoadingSpinner
          fullScreen
          text="正在连接实时服务器..."
  ...
  return (
    <div className="fixed inset-0 flex flex-col bg-[hsl(var(--background))] transition-colors duration-[var(--duration-normal)]">
      ...
      <MobilePullToRefresh
        ...
        className="flex-1 overflow-y-auto bg-[hsl(var(--background))] transition-colors duration-[var(--duration-normal)]"
      >
