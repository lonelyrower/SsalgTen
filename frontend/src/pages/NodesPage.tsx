...
const normalizeNodeEvent = (event: unknown): NodeEventRecord | null => {
  ...
  const id =
    typeof event.id === "string"
      ? event.id
      : String(event.id ?? `event-${Date.now()}`);
  ...
  const details = isObjectRecord(event.details)
    ? (event.details as NodeEventDetails)
    : undefined;
                  ...
                  <div className="space-y-3 flex-1 overflow-y-auto min-h-0">
                    ...
                    {filteredNodes.length > visibleCount && (
                      <button
                        ...
                        onClick={() =>
                          setVisibleCount((c) =>
                            Math.min(c + 60, filteredNodes.length),
                          )
                        }
                      >
                        加载更多
        ...
        {selectedNode && showServerDetails && (
          <div className="mt-6 transition-all duration-300 ease-in-out">
            ...
            <div className="mt-4 bg-white dark:bg-gray-800 rounded-lg shadow p-4">
              ...
              {/* 显示来自API的安全事件 */}
              {selectedNode?.securityEvents &&
                selectedNode.securityEvents.length > 0 && (
                  <div className="mb-4">
