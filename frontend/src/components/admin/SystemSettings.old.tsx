  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        const hasMapConfigChange = configsToUpdate.some((c) =>
          c.key.startsWith("map."),
        );
        if (hasMapConfigChange) {
          setSuccess(
            `成功更新了 ${configsToUpdate.length} 个配置项。地图配置已更新，3秒后自动刷新页面...`,
          );
          setTimeout(() => {
    ...
    <div className={className}>
      ...
      <div className="space-y-6">
        {filteredGroups.map((group) => {
          ...
          const colorScheme =
            colorSchemes[group.category as keyof typeof colorSchemes] ||
            defaultScheme;
          ...
          return (
            ...
            >
              {/* 优化的分组标题 - 添加渐变背景 */}
              <div
                className={`relative bg-gradient-to-r ${colorScheme.gradient} rounded-t-lg`}
              >
                <button
                    ...
                    <div className="flex items-center space-x-4 flex-1">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-3 mb-1.5">
                          ...
                          <span
                            className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold ${colorScheme.count}`}
                          >
                ...
                <div className="p-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                    {group.configs.map((config) => {
                      ...
                      const chineseDisplayName = getConfigDisplayName(
                        config.key,
                      );
                        ...
                        >
                          ...
                          {hasChanged && (
                            <div className="flex items-center justify-end pt-2 border-t border-gray-200 dark:border-gray-700">
                              <button
                                onClick={() => {
                                  const newChanges = new globalThis.Map(
                                    changedConfigs,
                                  );
              ...
              <div className="flex flex-col gap-2">
                ...
                <div className="flex items-start gap-2 text-xs text-gray-600 dark:text-gray-400">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {Array.from(changedConfigs.keys())
                      .slice(0, 5)
                      .map((key) => {
                        ...
                        return (
                          <span
                            key={key}
                            className="inline-flex items-center px-2 py-0.5 rounded-full bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300"
                          >
                            {displayName}
                          </span>
                        );
