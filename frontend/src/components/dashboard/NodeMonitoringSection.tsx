...
export const NodeMonitoringSection: React.FC = () => {
  ...
  // 流媒体数据存储
  const [streamingDataMap, setStreamingDataMap] = useState<
    Record<string, StreamingServiceResult[]>
  >({});
      ...
      try {
        ...
        if (response.success && response.data) {
          const serviceMap = new Map(
            response.data.services.map((service) => [service.service, service]),
          );
        ...
        console.error(
          `Failed to fetch streaming data for node ${nodeId}:`,
          error,
        );
      }
    },
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {filteredNodes.map((node) => (
            <NodeCardGrid
              key={node.id}
              node={node}
              streamingData={getStreamingData(node.id)}
            />
          ))}
        </div>
      ) : (
