...
export function useConnectivityDiagnostics(socketConnected: boolean): ConnectivityDiagnostics {
  ...
  const issues = useMemo(() => {
    ...
    if (!socketConnected) {
      items.push('实时连接未建立：检查 CORS_ORIGIN / FRONTEND_URL 配置是否匹配前端地址。');
    }
    if (apiReachable && nodesCount === 0) {
      items.push('系统暂无任何节点：确认 Agent 的 AGENT_API_KEY 与后端系统 API 密钥一致，且 Agent 可访问后端。');
    }
    return items;
