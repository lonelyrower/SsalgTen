...
- `DB_PASSWORD`: 强随机字符串（Docker Compose 必填）
...
- `POST /update`
  - Header: `X-Updater-Token: <UPDATER_TOKEN>`（必填）
...
- `UPDATER_TOKEN is required` 或 `DB_PASSWORD is required`：
  - 在生产 `.env` 中补齐必填变量后重新执行 `docker compose up -d`
